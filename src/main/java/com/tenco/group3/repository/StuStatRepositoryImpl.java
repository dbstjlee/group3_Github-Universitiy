...
public class StuStatRepositoryImpl implements StuStatRepository {
	...
	private static final String INSERT_STATUS_BY_ID_WITH_DES = " INSERT INTO stu_stat_tb (student_id, status, from_date, description) VALUES ";
	...
	@Override
	public int updateStatusById(List<Integer> idList, String status, String description) {
		int rowCount = 0;
		if (idList.size() == 0) {
			return rowCount;
		}
		...
		updateQuery.append(UPDATE_DATE);
		updateQuery.append("  WHERE student_id IN (");
		insertQuery.append(INSERT_STATUS_BY_ID_WITH_DES);
		for (int i = 0; i < idList.size(); i++) {
			updateQuery.append("?");
			insertQuery.append(" (?,?,CURDATE(),?)");
			if (i < idList.size() - 1) {
		...
		try (Connection conn = DBUtil.getConnection()) {
			...
			try (PreparedStatement pstmt = conn.prepareStatement(insertQuery.toString())) {
				...
				for (Integer integer : idList) {
					pstmt.setInt(count++, integer);
					pstmt.setString(count++, status);
					pstmt.setString(count++, description);
				}
				rowCount = pstmt.executeUpdate();
				conn.commit();
			} catch (Exception e) {
