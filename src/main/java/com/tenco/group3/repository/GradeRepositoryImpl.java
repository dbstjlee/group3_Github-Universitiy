...
public class GradeRepositoryImpl implements GradeRepository {
	...
	public List<Grade> getThisSemester(int studentId, int semester, int sub_year) {
		...
		try (Connection conn = DBUtil.getConnection();
			...
			try (ResultSet rs = pstmt.executeQuery()) {
				while (rs.next()) {
					Grade grade = Grade.builder().sub_year(rs.getInt("sub_year")).semester(rs.getInt("semester"))
	...
	@Override
	public List<Grade> getSemester(int studentId) {
		List<Grade> gradeList = new ArrayList<Grade>();
		try (Connection conn = DBUtil.getConnection();
	...
	public List<Grade> getSemesterByType(int studentId, Grade grade) {
		...
		try (Connection conn = DBUtil.getConnection();
			...
			pstmt.setInt(1, studentId);
			pstmt.setInt(1, grade.getSub_year());
			pstmt.setInt(1, grade.getSemester());
			pstmt.setString(1, grade.getType());
			try (ResultSet rs = pstmt.executeQuery()) {
				while (rs.next()) {
